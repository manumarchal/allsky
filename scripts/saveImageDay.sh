...
if [ "$UPLOAD_IMG" = true ] ; then
	...
	lftp "$PROTOCOL"://"$USER":"$PASSWORD"@"$HOST" -e "cd $IMGDIR; set net:max-retries 1; set net:timeout 20; put $FILENAME-resize.$EXTENSION; bye" &
	cp $FILENAME-resize.$EXTENSION /home/pi/allsky/images/$FILENAME-resize.$EXTENSION
fi
